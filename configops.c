     ...
     if ( !getStringToDelimiter( fileAccessPtr, COLON, dataBuffer )
         ...
         { 
          ...
          // set end state message to corrupt leader line error
             // function: copyString
          copyString( endStateMsg, "Corrupt configurationf leader line error" );

          // return coirrupt file data
          return false;
         }
      ...
      if( !getStringToDelimiter( fileAccessPtr, PERIOD, dataBuffer )
                                                                       ...
                                                                       != STR_EQ )
          {
           ...
           // set end state message to corrupt configuraiton end line
              // function: copyString
...
Function Ouput/Parameters: updated config leader line string (char *)
Function Output/Returned: configuration code value (ConfigCodeMessages)
Device Input/device: none 
Device Output/device: none 
...
void stripTrailingSpaces( char *str )
   {
    // initialize index to length of stirng - 1 to get highest array index
                              ...
                              double doubleVal, const char *lowerCaseStringVal )
   {
    ...
    switch( lineCode )
       ...
       case CFG_QUANT_CYCLES_CODE:
           ...
           // check for nquantum cycles limits exceeded
       ...
       case CFG_MEM_DISPLAY_CODE:
           ...
           // check for not finding either "on" or "offf"
              // function: compareString
