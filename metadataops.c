...
Dependencies: addNode, copyString, malloc
*/
...
Function Input/Parameters: node op code (OpCodeType *)
Function Output/Parameters: none
...
Process: screen dump/display of all op code items
    ...
    while( localPtr != NULL )
       {
        ...
        // assign local pointer to next in list
...
Function Output/Parameters: pointer to op code 
                            linked list head pointer (OpCodeType **),
...
bool getMetaData( const char *filename,
    ...
    *opCodeDataHead = NULL;
    ...
    // get start and end counts for later command
    ...
    while( accessResult == COMPLETE_OPCMD_FOUND_MSG )
       {
        ...
        // update start, and end counts for latter comparison 
          // function: updateStartCourt, updatEndCount
        startCount = updateStartCount( startCount, newNodePtr->strArg1 );
        endCount = updateEndCount( endCount, newNodePtr->strArg1 );
       }
...
/*
Name: getOpCommand
Process: acquires one op command line from a previously open file,
         parses it, and sets various struct members according
         to the three letter command
Function Input/Parameters: pointer to open file hande (FILE *)
...
Dependencies: getStringToDelimiter, getCommand, copyString, verifyValidCommand,
              compareString, getStringArg, verifyFirstArg, getNumberArg
*/
OpCodeMessages getOpCommand( FILE *filePtr, OpCodeType *inData )
   {
    ...
    // set all struct values that may not be initalized to defaults
    inData->pid = 0;
    ...
    // check for app start second argument
    if( compareString( inData->command, "app") == STR_EQ
                       && compareString( inData->strArg1, "start" ) == STR_EQ )
       {
        ...
        // set first int argumnet to number
    ...
    // check for memory base and offset
    else if( compareString( inData->command, "mem") == STR_EQ )
       {
        ...
        // set first second argumnet to number
...
Function Output/Parameters: pointer to captured integer value (int *)
...
Process: starts at given index, captures and assembles string argument,
         and returns as parameter
Function Input/Parameters: input string (const char *), starting index (int)
Function Output/Parameters: pointer to captured string value (char *)
Function Output/Returned: updated index for next function start
Device Input/device: none
Device Output/device: none
Dependencies: none
...
Process: checks for all possible of first argument string op command
...
Process: checks for all possible of three-letter op code command
...
bool verifyValidCommand( const char *testCmd )
   {
    // check for string holding correct three-letter op code command
      // function: compareString
