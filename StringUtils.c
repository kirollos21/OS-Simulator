...
Process: compares two strings with the following results:
         if the left string less than right string, returns less than zero
         if the left string greater than right string, returns greater than zero
         if the left string equals right string, returns zero
...
Device Input/Keyboard: none
Device Output/Monitor: none
Dependencies: getStringLength, malloc, free
*/
...
int findSubString( const char *testStr, const char *searchSubStr )
   {
    // initalize function/variables
      ...
      // initalize test string length
        // function: getStringLength
      int testStrLen = getStringLength( testStr );
        
      // initalize master index - location of the substring start point
      int masterIndex = 0;
        ...
        while( internalIndex <= testStrLen &&
              testStr[internalIndex] == searchSubStr[searchIndex] )
           {
            ...
            if( searchSubStr[searchIndex] == NULL_CHAR)
               {
                // return beginning location of sub strings
                return masterIndex;
               }
           }
    ...
    return SUBSTRING_NOT_FOUND; // temp
   }
...
Function Input/Parameters: input stream (FILE *),
                           ...
                           stops at specified delimiter (bool)
...
bool getStringConstrained( FILE *inStream, bool clearLeadingNonPrintable, 
                                         ...
                                         char delimiter, char *capturedString)
   {
    ...
    while( 
          // continues if not at end of the file and max string length not reached
          ( intChar != EOF && index < MAX_STR_LEN -1 )
        
          // AND
          // continues if not printable flag ser and character are printable
         ...
         )
       {
        ...
        // set next element to null character / end of string
        capturedString[index] = NULL_CHAR;
...
Process: finds the length of a string
         by counting the characters up to the NULL_CHAR character
...
int getStringLength( const char *string )
   {
    ...
    while( index < STD_STR_LEN && string[index] != NULL_CHAR)
       {
        // increase index forward
        index++;
       }
...
Process: captures a strings from the input stream
...
bool getStringToLineEnd( FILE *inStream, char *captureString )
   {
    ...
    return getStringConstrained( inStream, true, true, true,
                                          NON_PRINTABLE_CHAR, captureString);
   }

/*
Name: getSubString
Process: captures sub strings within larger strings
         between two inclusive indices.
         returns empty sting if either index is out of range,
...
void getSubString( char *destStr, const char *sourceStr,
                                                 int startIndex, int endIndex)
   {
    // initalize function/variables
      ...
      // initalize the destination index to zero
      int destIndex =0;
    ...
    if( startIndex >= 0 && startIndex <= endIndex && endIndex < sourceStrLen )
       {
        ...
        // loop across required substring (indices)
...
Device Input/Keyboard: none
Device Output/Monitor: none
Dependencies: toLowerCase, malloc, free
*/  
